...
import { View, Text,StyleSheet, Animated, Easing,ViewPropTypes, PanResponder } from 'react-native';
...
export default class Slider extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      thumbBackSize: { width: 0, height: 0 },
      allMeasured: false,
  ...
  handlePanResponderRequestEnd() {
    ...
    this.fireChangeEvent('changeValueFinish');
  }
  ...
  handleStartShouldSetPanResponder(e /* gestureState: Object */) {
    ...
    this.setState({onSilde:this.thumbHitTest(e)})
  ...
  handleMeasure(name, x) {
    ...
    if (this._containerSize && this._trackSize && this._thumbSize) {
      this.setState({
        ...
        thumbBackSize: this._thumbBackSize,
        allMeasured: true,
      });
    }
  }
  ...
  measureThumbBack = x => {
    this.handleMeasure('thumbBackSize', x);
  };
  ...
  getThumbBackPositionStyles(thumbStart) {
    let {thumbDiameter} = this.props
    if (this.props.orientation === 'vertical') {
      return [
        {
          translateX:
          (thumbDiameter?thumbDiameter/2:THUMB_SIZE/2) -(this.state.trackSize.height + this.state.thumbBackSize.height) / 2,
        },
        { translateY: thumbStart.__getValue()-((thumbDiameter?thumbDiameter:THUMB_SIZE)/4) },//必须调用组件的onValueChange函数
      ];
    }
    return [
      { translateX: thumbStart.__getValue()-(this.state.thumbBackSize.width-(thumbDiameter?thumbDiameter:THUMB_SIZE))/2 },
      {
        translateY:-(this.state.trackSize.height + this.state.thumbBackSize.height-(thumbDiameter?thumbDiameter:THUMB_SIZE)/4) / 2+this.state.thumbSize.height/2,
      },
    ];
  }
  ...
  render() {
    ...
    const { value, containerSize, thumbSize,thumbBackSize, allMeasured } = this.state;
    ...
    return (
      ...
      >
        ...
        {
            <Animated.View
            onLayout={this.measureThumbBack}
            style={StyleSheet.flatten([
                orientation === 'vertical'? mainStyles.thumbBackAreaVer:mainStyles.thumbBackAreaHor,
              {
                transform: [
                  ...this.getThumbBackPositionStyles(thumbStart),
                  ...thumbStyleTransform,
                ],
                // ...valueVisibleStyle,
              },
              { width:orientation === 'vertical'?thumbDiameter?thumbDiameter*3: THUMB_SIZE*3:thumbDiameter?thumbDiameter+10:THUMB_SIZE+10,height:orientation === 'vertical'?thumbDiameter?thumbDiameter+10:THUMB_SIZE+10:thumbDiameter?thumbDiameter*3: THUMB_SIZE*3,
                backgroundColor: showValue && this.state.onSilde?"rgba(216, 216, 216, 0.4)":'transparent' },
              // { backgroundColor: "rgba(216, 216, 216, 0.4)" }, 
            ])}
          >
            {
              orientation === 'vertical'? <Text style={{textAlign:"left",paddingLeft:scaleSize(20),display:showValue && this.state.onSilde?'flex':'none',color:valueColor}}>{acturalValue||acturalValue==0?acturalValue:parseInt(this.getCurrentValue())}</Text>: <Text style={{textAlign:"center",display:showValue && this.state.onSilde?'flex':'none',color:valueColor}}>{acturalValue||acturalValue==0?acturalValue:parseInt(this.getCurrentValue())}</Text>
            }
          </Animated.View>
        }
        <Animated.View
...
Slider.propTypes = {
  ...
  valueColor: PropTypes.string,
    /**
   * 滑动的小圆点的直径，用这个来计算显示滑动值区域的宽高
   */
  thumbDiameter: PropTypes.number,
  /**
   * 真实的滑动数值
   */
  acturalValue:PropTypes.number||PropTypes.string,
  /**
